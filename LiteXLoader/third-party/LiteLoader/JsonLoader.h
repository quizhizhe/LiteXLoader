...
struct JsonReader {
    Document dc;
    JsonReader() {}
    JsonReader(const char *fn) { read(fn); }
    Document &read(const char *fn) {
        ...
        dc.Parse(v.c_str());
        if (dc.HasParseError()) {
            throw string("json err [") + rapidjson::GetParseError_En(dc.GetParseError()) + "] at " +
                std::to_string(dc.GetErrorOffset());
        }
        return dc;
    }
};
